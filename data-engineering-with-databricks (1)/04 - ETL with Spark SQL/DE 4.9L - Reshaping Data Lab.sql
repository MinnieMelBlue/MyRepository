...
SELECT * FROM (
  SELECT user_id user, event_name --Group by user
  FROM events
) PIVOT(count(*) FOR event_name IN --pivot on event_name to provide a count of every event type in its own column
